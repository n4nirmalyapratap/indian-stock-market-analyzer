 ...
 * Mobile (< md): simple prev/next quarter navigator + card rows with
 *   visual % bars and delta chips. No horizontal scroll.
 */
...
// ── Mobile: quarter navigator ─────────────────────────────────────────────
// Simple prev/next — no scrollable pill strip that goes off-screen.
      ...
      <div className="flex flex-wrap gap-x-3 gap-y-1">
        {segments.map((s) => (
          <div key={s.key} className="flex items-center gap-1 text-xs">
            ...
            <span className={`font-semibold tabular-nums ${textColorMap[s.dotClass] ?? ""}`}>
              {s.pct.toFixed(1)}%
            </span>
          </div>
        ))}
...
export default function ShareholdingPattern({ symbol }: Props) {
  ...
  const mobilePrev = ordered[safeQIdx + 1] ?? null;
          ...
          <>
            ...
            {mobilePrev && (
              <p className="px-4 pt-2 text-[11px] text-gray-400 dark:text-gray-500">
                Δ vs {quarterLabel(mobilePrev.asOnDate)}
              </p>
            )}
            ...
            <div className="divide-y divide-gray-50 dark:divide-gray-700/60">
              {visibleBuckets.map((b) => (
                <BucketCard
                  ...
                  prevValue={mobilePrev ? (mobilePrev[b.key] ?? null) : null}
              ...
              {showPledge && (
                <PledgeCard
                  ...
                  prevValue={mobilePrev?.promoterPledgePct ?? null}
                />
              )}
