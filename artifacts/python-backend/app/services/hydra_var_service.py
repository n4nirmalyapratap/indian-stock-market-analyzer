...
def historical_var(
    ...
    var_pct = float(np.percentile(rets_arr, pct))   # e.g. 5th percentile
    ...
    return {
        ...
        "varAbsolute":     round(var_abs, 2),
        "cvarAbsolute":    round(cvar_abs, 2),
...
def portfolio_var(
    ...
    valid_syms = [s for s in symbols if s in returns_by_sym]
    if len(valid_syms) > 1:
        matrix = np.array([
            np.array(returns_by_sym[s][-min_len:]) for s in valid_syms
        ])
        corr = np.corrcoef(matrix)
        corr_matrix = [[round(float(corr[i][j]), 3) for j in range(len(valid_syms))]
                       for i in range(len(valid_syms))]

    # Individual VaRs for breakdown
    breakdown = []
    for sym, w in zip(symbols, weights):
    ...
    return {
        ...
        "portfolioVarAbs":  round(var_abs, 2),
        "portfolioCvarAbs": round(cvar_abs, 2),
        "portfolioValue":   portfolio_value,
        "confidence":       confidence,
        "horizonDays":      horizon_days,
        "sampleSize":       min_len,
